...
function buildSenaryolar() {
  ...
  var cardsHtml = [{lbl:'Kötümser',g:12,rk:'var(--red)',bg:'var(--red-bg)',bd:'var(--red-bd)'},{lbl:'Gerçekçi ✓',g:18,rk:'var(--green)',bg:'var(--green-bg)',bd:'var(--green-bd)',b:true},{lbl:'İyimser',g:25,rk:'var(--blue)',bg:'var(--blue-bg)',bd:'var(--blue-bd)'}].map(function(s){var v=calcPortfoy(S.yatirim,s.g,kalan,P.artis||0);return'<div style="background:'+s.bg+';border:1px solid '+s.bd+';border-radius:8px;padding:10px 12px;display:flex;justify-content:space-between;align-items:center;"><div><div style="font-size:10px;font-weight:700;color:'+s.rk+';margin-bottom:2px;">'+s.lbl+'</div><div style="font-size:10px;color:var(--ink3);">%'+s.g+' yıllık</div></div><div style="font-size:'+(s.b?'15':'13')+'px;font-weight:700;color:'+s.rk+';">'+fmt(v)+'</div></div>';}).join('');
  document.getElementById('senaryo-cards').innerHTML = cardsHtml + '<div style="margin-top:16px; padding:12px 8px 4px 8px; border-top:1px solid var(--border); text-align:center;"><i data-lucide="info" class="lu" style="width:14px; height:14px; color:var(--ink3); margin-bottom:6px; display:inline-block; opacity:0.8;"></i><div style="font-size:10px; color:var(--ink3); line-height:1.5; max-width:260px; margin:0 auto; font-style:italic;">Hesaplamalar enflasyondan arındırılmamış <b>nominal (cari)</b> değerlerdir; gelecekteki alım gücü bugünden farklı olabilir.</div></div>';
